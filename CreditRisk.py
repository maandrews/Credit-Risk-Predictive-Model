...
for ds in combine:
    for i in range (0, ds['age'].max()+1):
        ds.loc[(ds.MonthlyIncome.isnull()) & (ds.age == i),'MonthlyIncome'] = guess_income[i]
...
for ds in combine:
    ...
    ds['WeightedMissedPayments'] = 0.5*ds.NumberOfTimes90DaysLate + 0.3*ds['NumberOfTime60-89DaysPastDueNotWorse'] \
...
next_x_train = np.concatenate((predictions_LR_train,predictions_DT_train, predictions_NN_train, predictions_KNN_train,
                               predictions_RF_train, predictions_AB_train, predictions_GNB_train), axis=1)
